...
// LOGIKA GLOW SETELAH AKSI (tanpa auto-scroll, cukup efek cahaya nya)
useEffect(() => {
  if (highlightTx) {
...
const filteredLedger = serverLedger;
  ...
  // 🆕 Auto-detect kategori unik dari semua transaksi
const uniqueCategories = useMemo(() => {
  const categories = new Set<string>();
  
  // Ambil semua kategori dari transactions props (semua data)
  transactions.forEach(t => {
    if (t.category && t.category.trim()) {
      categories.add(t.category.toUpperCase());
    }
  });
  
  // Convert Set jadi Array & sort alfabetis
...
{showAddModal && (
        <ModalPortal>
  <div data-modal-container className="fixed inset-0 z-[100000] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-6 opacity-0" style={{animation: 'modalFadeIn 0.3s ease-out forwards'}}>
     <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl relative overflow-hidden opacity-0" style={{animation: 'modalZoomIn 0.3s ease-out 0.1s forwards'}}>
               <div className="p-10 md:p-12">
               <button onClick={() => setShowAddModal(false)} className="absolute top-10 right-10 z-10 p-2 text-slate-300 hover:text-rose-500 transition-colors"><X size={22}/></button>
               <h4 className="text-2xl font-black text-slate-800 uppercase italic mb-8 tracking-tighter text-center">Input <span className="text-blue-600">Manual</span></h4>
  ...
  <div data-modal-container className="fixed inset-0 z-[100000] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-6 opacity-0" style={{animation: 'modalFadeIn 0.3s ease-out forwards'}}>
     <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-2xl relative overflow-hidden opacity-0" style={{animation: 'modalZoomIn 0.3s ease-out 0.1s forwards'}}>
               <div className="p-10 md:p-12">
               <button onClick={() => setEditingTransaction(null)} className="absolute top-10 right-10 z-10 p-2 text-slate-300 hover:text-rose-500 transition-colors"><X size={22}/></button>
               <h4 className="text-2xl font-black text-slate-800 uppercase italic mb-8 tracking-tighter text-center">Edit <span className="text-blue-600">Transaksi</span></h4>
...
<div className="flex flex-col items-center mb-8 text-center">
  ...
  <div>
    <h4 className="text-2xl font-black text-slate-800 uppercase italic leading-none">Import Data Kas</h4>
