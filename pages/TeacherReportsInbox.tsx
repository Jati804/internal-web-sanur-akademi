...
const TeacherReportsInbox: React.FC<TeacherReportsInboxProps> = ({ user, logs, studentAccounts, refreshAllData }) => {
  ...
  const publishedReports = useMemo(() => {
    ...
    return sorted.filter(req => {
        ...
        return sName.includes(term) || cName.includes(term) || avg.toString().includes(term) || statusText.includes(term) || dateStr.includes(term);
    });
    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-40 px-4 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-10 px-2">
        <div className="space-y-4">
           <h2 className="text-5xl font-black text-slate-800 tracking-tight leading-none uppercase italic">Portal <span className="text-orange-600">Rapot</span></h2>
           <p className="text-[11px] font-black text-slate-400 uppercase tracking-widest italic">Terbitkan Sertifikat & Rapot Digital Siswa ✨</p>
        </div>
      </header>
      ...
      <div className="bg-orange-50 p-6 rounded-[2.5rem] border border-orange-100 flex items-center gap-4 animate-pulse shadow-sm max-w-2xl mx-auto">
         ...
         <p className="text-[10px] font-black text-orange-800 uppercase italic">"Info: Jika rapot siswa sudah keluar, ingatkan mereka segera mengamankan filenya ya Kak! Karena akun siswa akan dihapus pengurus setelah siswa satu tahun lulus untuk meringankan sistem. ✨"</p>
      </div>

      <div className="flex bg-slate-100 p-2 rounded-[2.5rem] w-full max-w-xl mx-auto shadow-inner border border-slate-200">
         <button onClick={() => setActiveStep('ANTREAN')} className={`flex-1 py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-widest transition-all ${activeStep === 'ANTREAN' ? 'bg-white text-orange-600 shadow-xl' : 'text-slate-400'}`}>Antrean ({reportRequests.length})</button>
         <button onClick={() => setActiveStep('WORKSPACE')} disabled={!selectedPackage} className={`flex-1 py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-widest transition-all ${activeStep === 'WORKSPACE' ? 'bg-white text-blue-600 shadow-xl' : 'text-slate-400 disabled:opacity-30'}`}>Workspace</button>
         <button onClick={() => setActiveStep('HISTORY')} className={`flex-1 py-5 rounded-[2rem] text-[11px] font-black uppercase tracking-widest transition-all ${activeStep === 'HISTORY' ? 'bg-white text-emerald-600 shadow-xl' : 'text-slate-400'}`}>Histori</button>
      </div>

      {activeStep === 'HISTORY' && (
         <div className="relative max-w-2xl mx-auto mt-8 shadow-2xl shadow-slate-200/50 animate-in slide-in-from-top-4">
            <Search size={22} className="absolute left-8 top-1/2 -translate-y-1/2 text-emerald-500" />
            <input type="text" placeholder="CARI NAMA, MATPEL, NILAI, ATAU STATUS..." value={historySearchTerm} onChange={e => setHistorySearchTerm(e.target.value.toUpperCase())} className="w-full pl-16 pr-8 py-6 bg-white border border-slate-100 rounded-full text-[13px] font-black uppercase outline-none shadow-sm focus:border-emerald-500 transition-all placeholder:text-slate-300" />
         </div>
      )}
      ...
      {activeStep === 'ANTREAN' && (
         <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-10 max-w-6xl mx-auto">
            {reportRequests.map((req, i) => (
              <div key={i} className="bg-white p-12 md:p-16 rounded-[4.5rem] shadow-2xl border border-slate-50 flex flex-col justify-between hover:border-orange-500 transition-all group min-h-[480px]">
                 <div>
                   <div className="flex justify-between items-start mb-10">
                      <div className="w-20 h-20 bg-orange-50 text-orange-600 rounded-[2.5rem] flex items-center justify-center shadow-inner group-hover:bg-orange-600 group-hover:text-white transition-all duration-500"><GraduationCap size={44}/></div>
                      <span className="px-6 py-2.5 bg-orange-500 text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl animate-pulse">PERMINTAAN BARU ✨</span>
                   </div>
                   <h4 className="text-3xl font-black text-slate-800 uppercase italic mb-3 leading-none tracking-tight">{req.studentsAttended?.[0]}</h4>
                   <p className="text-[12px] font-bold text-blue-600 uppercase mb-12 tracking-wide">{req.className}</p>
                   <button onClick={() => setShowMilestoneFor(req)} className="w-full py-5 mb-6 bg-slate-50 text-slate-500 rounded-[2rem] font-black text-[10px] uppercase flex items-center justify-center gap-3 hover:bg-blue-600 hover:text-white transition-all border border-transparent shadow-sm"><History size={20}/> LIHAT MILESTONE LENGKAP</button>
                 </div>
                 <div className="space-y-4">
                    <button onClick={() => handleAcceptRequest(req)} className="w-full py-8 bg-slate-900 text-white rounded-[2.5rem] font-black text-[13px] uppercase tracking-[0.4em] shadow-2xl active:scale-95 transition-all hover:bg-black group/btn flex items-center justify-center gap-4">
                       ISI RAPOT SISWA <ChevronRight size={24} className="group-hover/btn:translate-x-2 transition-transform" />
                    </button>
                 </div>
              </div>
            ))}
            {reportRequests.length === 0 && (
               <div className="col-span-full py-48 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 opacity-30">
                  <UserCheck size={64} className="mx-auto mb-6 text-slate-300" />
                  <p className="font-black text-[12px] uppercase tracking-[0.4em] italic leading-relaxed text-center text-slate-400">Belum ada antrean rapot baru. ✨</p>
               </div>
            )}
         </div>
      )}
      ...
      {activeStep === 'WORKSPACE' && selectedPackage && (
         <div className="bg-white rounded-[4rem] shadow-2xl border-4 border-blue-600 overflow-hidden animate-in zoom-in space-y-0 max-w-6xl mx-auto">
            ...
            <div className="p-8 md:p-14 space-y-16">
               <section className="space-y-4">
                  ...
                  <div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 shadow-inner"><MilestoneView logs={logs} studentName={selectedPackage.studentsAttended?.[0] || ''} packageId={selectedPackage.packageId} /></div>
               </section>
                     ...
                     {reportForm.sessions.map((s, i) => (
                        <div key={i} className="flex flex-col gap-4 bg-slate-50 p-8 rounded-[2.5rem] border-2 border-transparent focus-within:border-blue-500 transition-all shadow-inner">
                           ...
                           <div className="space-y-2">
                              ...
                              <input type="text" placeholder="MISAL: PENGENALAN TOOLS..." value={s.material} maxLength={35} onChange={e => { const n = [...reportForm.sessions]; n[i].material = e.target.value; setReportForm({...reportForm, sessions: n}); }} className="w-full bg-white px-5 py-3 rounded-xl font-black uppercase text-[11px] outline-none border border-slate-200 focus:border-blue-500 transition-all" />
                           </div>
                        </div>
                     ))}
                  </div>
               ...
               <section className="pt-10">
                  <button onClick={handlePublishReport} disabled={loading} className="w-full py-10 bg-emerald-600 text-white rounded-[3rem] font-black text-[15px] uppercase tracking-[0.4em] shadow-2xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-5 active:scale-95">
                     {loading ? <Loader2 size={32} className="animate-spin" /> : <><Sparkles size={32} /> {isEditMode ? 'SIMPAN PERUBAHAN RAPOT ✨' : 'TERBITKAN RAPOT DIGITAL ✨'}</>}
                  </button>
               </section>
            </div>
         </div>
      ...
      {activeStep === 'HISTORY' && (publishedReports.length > 0 ? (
         <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-10 max-w-6xl mx-auto">
            {publishedReports.map((req, i) => {
               ...
               const isPass = avg >= 80;
               return (
                  <div key={i} className="bg-white p-12 md:p-14 rounded-[4.5rem] shadow-2xl border border-slate-50 flex flex-col hover:border-emerald-500 transition-all group min-h-[460px]">
                     <div className="flex justify-between items-start mb-10">
                        <div className={`w-16 h-16 rounded-[2rem] flex items-center justify-center shadow-inner shrink-0 ${isPass ? 'bg-emerald-50 text-emerald-600' : 'bg-orange-50 text-orange-600'}`}>{isPass ? <BadgeCheck size={36}/> : <AlertCircle size={36}/>}</div>
                        <div className="flex gap-3 items-center">
                           <button onClick={() => handleOpenWorkspace(req, true)} className="p-4 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition-all shadow-sm"><FileEdit size={22}/></button>
                           <span className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-md flex items-center justify-center ${isPass ? 'bg-emerald-600 text-white' : 'bg-orange-500 text-white'}`}>{isPass ? 'LULUS 🎓' : 'REMEDIAL'}</span>
                        </div>
                     </div>
                     <h4 className="text-3xl font-black text-slate-800 uppercase italic mb-2 truncate leading-none tracking-tight">{sName}</h4>
                     <p className="text-[12px] font-bold text-blue-600 uppercase mb-10 truncate tracking-wide">{req.className}</p>
                     <div className="bg-slate-50 p-8 rounded-[3.5rem] mb-12 flex justify-between items-center border border-slate-100/50 shadow-inner">
                        <div><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Rerata Nilai</p><p className={`text-4xl font-black italic ${isPass ? 'text-emerald-600' : 'text-orange-600'}`}>{avg}</p></div>
                        <div className="text-right"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Tanggal Terbit</p><p className="text-[14px] font-black text-slate-800 uppercase tracking-normal">{formatDateToDMY(req.date)}</p></div>
                     </div>
                     <button onClick={() => handleDownloadPDF(req)} disabled={activeDownloadId === req.id} className="w-full py-7 bg-slate-900 text-white rounded-[2.5rem] font-black text-xs uppercase tracking-widest flex items-center justify-center gap-4 hover:bg-emerald-600 transition-all shadow-2xl active:scale-95">
                        {activeDownloadId === req.id ? <Loader2 className="animate-spin" size={24} /> : <Printer size={24}/>} CETAK RAPOT DIGITAL ✨
                     </button>
                  </div>
               );
            })}
         ...
         <div className="col-span-full py-40 text-center bg-white rounded-[4rem] border-2 border-dashed border-slate-100 opacity-30 max-w-xl mx-auto">
            <History size={64} className="mx-auto mb-6 text-slate-300" />
            <p className="font-black text-[12px] uppercase tracking-[0.4em] italic leading-relaxed text-center text-slate-400">Belum ada histori rapot{historySearchTerm ? ' yang sesuai' : ''}. ✨</p>
         </div>
      ))}

      {/* RENDERER PDF TERSEMBUNYI (LOCK WIDTH 794px) */}
      <div className="fixed left-[-9999px] top-0 pointer-events-none" style={{ width: '794px' }}>
         {publishedReports.map((req) => {
            ...
            return (
              <div key={req.id} id={`pdf-group-${req.id}`} style={{ width: '794px' }}>
                 <div id={`cert-render-${req.id}`} className={`w-[794px] h-[1123px] bg-white flex flex-col border-[25px] border-double box-border p-12 relative overflow-hidden ${isPass ? 'border-blue-900' : 'border-orange-600'}`}>
                    <div className="w-full h-full border-4 border-slate-100 flex flex-col items-center py-12 box-border relative z-10">
                      <div className="flex flex-col items-center justify-start pt-14 mb-8">
                        ...
                        <h1 className={`text-2xl font-black tracking-[0.25em] uppercase leading-none ${isPass ? 'text-blue-900' : 'text-orange-900'}`}>SANUR AKADEMI INSPIRASI</h1>
                      ...
                      <div className="flex flex-col items-center text-center w-full px-12 flex-1 justify-center -mt-32">
                        <h2 className={`text-5xl font-serif italic mb-8 leading-tight tracking-wide ${isPass ? 'text-blue-900' : 'text-orange-900'}`}>{isPass ? 'Sertifikat Kelulusan' : 'Laporan Capaian Belajar'}</h2>
                        <p className="text-xl font-serif italic text-slate-500 mb-8 tracking-wider">{isPass ? 'Dengan bangga diberikan kepada:' : 'Diberikan kepada:'}</p>
                        <div className="relative mb-12 w-full"><h3 className={`text-5xl font-black uppercase tracking-[0.08em] text-center px-6 leading-none drop-shadow-sm ${isPass ? 'text-blue-600' : 'text-orange-600'}`}>{sName}</h3><div className={`w-full h-1.5 mt-6 rounded-full mx-auto max-w-[60%] ${isPass ? 'bg-blue-100' : 'bg-orange-100'}`}></div></div>
                        <p className="text-lg font-serif italic text-slate-600 px-20 leading-relaxed mb-10 tracking-wide">{isPass ? 'Telah menunjukkan kompetensi luar biasa and berhasil menyelesaikan kurikulum pelatihan intensif dengan hasil memuaskan pada program:' : 'Telah berpartisipasi aktif and menyelesaikan seluruh modul kurikulum pelatihan intensif dengan dedikasi tinggi pada program:'}</p>
                        <div className={`bg-gradient-to-br px-16 py-8 rounded-[3rem] border-4 border-white shadow-2xl flex flex-col items-center justify-center min-w-[520px] ${isPass ? 'from-blue-900 to-slate-900' : 'from-orange-600 to-slate-900'}`}><p className="text-2xl font-black text-white uppercase italic tracking-[0.1em] text-center leading-tight">{subject}</p><p className={`text-[10px] font-black tracking-[0.7em] uppercase mt-3 ${isPass ? 'text-blue-300' : 'text-orange-200'}`}>LEVEL {level}</p></div>
                      </div>
                      <div className="w-full flex flex-col items-center relative z-20">
                        <div className="w-full px-20 absolute bottom-[22px] left-0 flex justify-between items-center z-10 gap-10">
                          <div className="flex-1 flex flex-col items-center text-center"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic mb-1">Tanggal Terbit:</p><p className="text-base font-black text-slate-900 uppercase tracking-tighter">{formatDateToDMY(req.date)}</p></div>
                          <div className="flex-1 flex flex-col items-center justify-center text-blue-900/10 opacity-40 shrink-0"><BookOpen size={60} /><div className="flex gap-1.5 mt-1"><Star size={10} fill="currentColor"/><Star size={10} fill="currentColor"/><Star size={10} fill="currentColor"/></div></div>
                          <div className="flex-1 flex flex-col items-center text-center"><div className="p-3 border-4 border-slate-100 rounded-3xl bg-white shadow-sm flex items-center justify-center mx-auto"><img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=VERIFIKASI%20SERTIFIKAT%20SANUR%0AID:%20${req.id.substring(0,8).toUpperCase()}%0ANAMA:%20${sName}%0APROGRAM:%20${subject}%0ASTATUS:%20TERVERIFIKASI%20RESMI`} className="w-16 h-16" alt="QR" /></div><p className="text-[7px] font-black text-slate-400 uppercase tracking-widest text-center mt-1">Verifikasi Sertifikat</p></div>
                        </div>
                      </div>
                    </div>
                 </div>
                 <div id={`transcript-render-${req.id}`} className="w-[794px] h-[1123px] bg-white p-12 flex flex-col relative border-8 border-slate-100 box-border overflow-hidden">
                    <div className="flex justify-between items-start mb-6"><div className="flex items-center gap-4"><div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white shadow-lg ${isPass ? 'bg-blue-600' : 'bg-orange-600'}`}><Layout size={28}/></div><div><h1 className={`text-3xl font-black italic uppercase leading-none tracking-tighter ${isPass ? 'text-blue-600' : 'text-orange-600'}`}>SANUR</h1><p className="text-[9px] font-black uppercase text-slate-400 tracking-[0.4em] mt-1">Catatan Akademik Resmi</p></div></div><div className="text-right"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Kode Referensi</p><p className="text-base font-black text-slate-800">SN/TR/{req.id.substring(0,8).toUpperCase()}</p></div></div>
                    <div className="grid grid-cols-2 gap-6 mb-6 bg-slate-50 p-6 rounded-[2rem] border border-slate-200"><div className="border-r border-slate-300"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Nama Siswa</p><p className="text-xl font-black text-slate-800 uppercase italic leading-none">{sName}</p></div><div className="pl-5"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Program Akademik</p><p className={`text-xl font-black uppercase italic leading-tight ${isPass ? 'text-blue-600' : 'text-orange-600'}`}>{subject}</p></div></div>
                    <div className="h-auto rounded-[2.5rem] border-2 border-slate-300 overflow-hidden mb-6 bg-white"><table className="w-full table-fixed border-collapse bg-white"><thead><tr className="bg-slate-900 text-white"><th className="p-4 text-center text-[10px] font-black uppercase tracking-widest border-r border-white/10 w-24">Sesi</th><th className="p-4 text-left text-[10px] font-black uppercase tracking-widest">Materi & Modul Kurikulum</th><th className="p-4 text-center text-[10px] font-black uppercase tracking-widest w-32">Nilai</th></tr></thead><tbody className="bg-white">{[1,2,3,4,5,6].map((num, i) => (<tr key={i} className="bg-white border-b border-slate-100 last:border-none"><td className="w-24 border-r border-slate-200 p-0 h-[100px] text-center"><span className="font-black text-slate-200 text-3xl italic">0{num}</span></td><td className="p-0 h-[100px] px-10"><span className="font-black text-slate-800 text-[18px] uppercase italic tracking-tight">{topics[i] || "MATERI PEMBELAJARAN"}</span></td><td className="w-32 p-0 h-[100px] text-center"><div className="flex items-baseline gap-1 justify-center"><span className={`font-black text-4xl italic ${isPass ? 'text-blue-600' : 'text-orange-600'}`}>{scores[i] || 0}</span><span className="text-slate-300 font-bold text-sm uppercase">/100</span></div></td></tr>))}</tbody></table></div>
                    <div className="p-10 bg-slate-900 rounded-[3rem] text-white flex justify-between items-center shadow-2xl relative overflow-hidden"><div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-32 -mt-32"></div><div className="space-y-0 relative z-10 flex flex-col justify-center"><p className={`text-[10px] font-black uppercase tracking-[0.5em] mb-0 ${isPass ? 'text-blue-400' : 'text-orange-400'}`}>Evaluasi Kumulatif</p><div className="flex items-baseline gap-4 -mt-14"><p className="text-[18px] font-black text-white/40 uppercase tracking-widest">RATA-RATA:</p><h4 className="text-7xl font-black italic tracking-tighter text-white leading-tight">{avg}</h4></div></div><div className="bg-white/10 p-6 rounded-[1.8rem] border border-white/20 text-center backdrop-blur-md relative z-10 min-w-[200px] overflow-hidden"><p className={`text-[10px] font-black uppercase mb-1.5 tracking-widest ${isPass ? 'text-blue-300' : 'text-orange-300'}`}>Status Capaian</p><p className="text-xl font-black italic tracking-tighter uppercase text-white mb-2">{isPass ? 'KOMPETEN ✨' : 'REMEDIAL'}</p><div className={`absolute bottom-0 left-0 h-1.5 w-full ${isPass ? 'bg-emerald-500 shadow-[0_0_12px_rgba(16,185,129,0.8)]' : 'bg-orange-500 shadow-[0_0_12px_rgba(249,115,22,0.8)]'}`}></div></div></div>
                 </div>
                 <div id={`milestone-render-${req.id}`} className="w-[794px] h-[1123px] bg-white p-20 pb-40 flex flex-col border relative overflow-hidden box-border">
                    <div className={`absolute top-0 left-0 w-[400px] h-[400px] rounded-full -ml-48 -mt-48 opacity-50 ${isPass ? 'bg-blue-50' : 'bg-orange-50'}`}></div>
                    <div className="flex items-center gap-4 mb-10 relative z-10"><div className="w-14 h-14 bg-slate-900 text-white rounded-[1.2rem] flex items-center justify-center shadow-2xl"><Zap size={28}/></div><h1 className="text-4xl font-black italic text-slate-800 uppercase leading-none tracking-tighter">Langkah <span className={isPass ? 'text-blue-600' : 'text-orange-600'}>Pembelajaran</span></h1></div>
                    <div className="space-y-8 mb-12 relative z-10"><div className={`flex items-center gap-5 border-b-4 pb-3 ${isPass ? 'text-blue-600 border-blue-50' : 'text-orange-600 border-orange-50'}`}><ClipboardList size={24}/><h3 className="text-base font-black uppercase tracking-[0.5em]">Log Pelaksanaan Sesi</h3></div><div className="space-y-4">{[1,2,3,4,5,6].map((num, i) => { const l = pkgLogs.find(x => x.sessionNumber === num); return (<div key={i} className="flex items-center gap-12 px-12 py-3 border-b border-slate-50 last:border-none group"><div className={`font-black text-xl uppercase italic w-24 shrink-0 ${SESSION_COLORS[i] || 'text-slate-400'}`}>SESI {num}</div><div className="flex-1 flex items-center justify-between"><p className="text-[13px] font-black text-slate-400 uppercase tracking-widest">{l ? formatDateToDMY(l.date) : "—"}</p><p className="text-[14px] font-black text-slate-800 uppercase italic">Durasi: 2 JAM / 120 MENIT</p></div></div>); })}</div></div>
                    <div className="space-y-6 relative z-10 flex-1 flex flex-col mb-16"><div className={`flex items-center gap-5 border-b-4 pb-3 ${isPass ? 'text-blue-600 border-blue-50' : 'text-orange-600 border-orange-50'}`}><Quote size={24}/><h3 className="text-base font-black uppercase tracking-[0.5em]">Ulasan Pengajar</h3></div><div className="flex-1 bg-gradient-to-br from-slate-50 to-white rounded-[3rem] border-4 border-slate-100 p-10 flex flex-col items-center justify-center text-center relative overflow-hidden"><p className="text-lg font-serif italic text-slate-700 leading-relaxed px-8 relative z-10 drop-shadow-sm break-words whitespace-normal overflow-wrap-anywhere max-w-full">"{nar}"</p></div></div>
                    <div className="mt-auto flex flex-col items-center opacity-30 pt-10 relative z-10 border-t-2 border-slate-100 pb-28"><p className="text-sm font-black uppercase tracking-[0.8em] text-slate-400">Dokumen Resmi Sanur Akademi</p></div>
                 </div>
              </div>
            );
         })}
