...
const handleDownloadPDFReport = async (course: any) => {
  ...
  const passColor = '#1e3a8a';
  const failColor = '#ea580c';
  const mainColor = isPass ? passColor : failColor;
...
<head>
  <meta charset="UTF-8">
  <title>Rapot_Sanur_${normalizedUserName.replace(/\s+/g, '_')}</title>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    body { font-family: 'Inter', serif; background: white; }
...
<div class="no-print" style="position:fixed; top:16px; right:16px; z-index:9999; display:flex; gap:10px;">
  <div style="background:#0f172a; color:white; padding:10px 20px; border-radius:12px; font-weight:900; font-size:12px; text-transform:uppercase; letter-spacing:0.1em;">
    ⬆️ Klik File → Print → Save as PDF
  </div>
...
<div class="page-portrait">
  ...
  <div style="margin-bottom:30px; display:flex; justify-content:space-between; align-items:flex-end;">
    ...
    <div style="display:flex; flex-direction:column; align-items:flex-end; text-align:right;">
      ...
      <p style="font-size:13px; font-weight:900; color:#1e293b; text-transform:uppercase; margin:0; letter-spacing:0.05em;">${reportLog.teacherName || '-'}</p>
    </div>
  </div>
