      ...
      {isPaymentView ? (
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-10">
           {verifiedCourses.map((course, idx) => {
              ...
              return (
                <div key={course.id || idx} className="bg-white rounded-[4rem] border-2 border-slate-50 shadow-2xl flex flex-col hover:border-emerald-500 transition-all duration-500 overflow-hidden">
                   <div className="p-10 md:p-12 flex flex-col flex-1 space-y-10"><div className="flex justify-between items-start"><div className={`w-20 h-20 ${isReportPublished ? (isPass ? 'bg-emerald-600' : 'bg-orange-600') : isRejected ? 'bg-rose-600' : 'bg-blue-600'} text-white rounded-[2rem] flex items-center justify-center shadow-xl rotate-3`}>{isReportPublished ? <BadgeCheck size={40} /> : isRejected ? <AlertTriangle size={40} /> : <BookOpen size={40} />}</div><span className={`px-6 py-2.5 ${isReportPublished ? (isPass ? 'bg-emerald-600' : 'bg-orange-600') : isRejected ? 'bg-rose-600' : isProcessing ? 'bg-orange-500' : 'bg-blue-600'} text-white rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center justify-center`}>{isReportPublished ? (isPass ? 'LULUS 🎓' : 'SELESAI ✨') : isRejected ? 'DITOLAK GURU' : isProcessing ? 'PROSES RAPOT' : 'PAKET AKTIF ✨'}</span></div><div className="space-y-2"><h3 className="text-3xl font-black text-slate-800 uppercase italic tracking-normal leading-tight">{course.className}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest italic">Lifetime Access Certificate</p></div>{isReportPublished ? (<div className={`${isPass ? 'bg-emerald-600' : 'bg-orange-600'} p-10 rounded-[3rem] text-white flex flex-col items-center text-center space-y-6 shadow-2xl animate-in zoom-in duration-500`}><div className="w-20 h-20 bg-white/20 rounded-full flex items-center justify-center animate-bounce">{isPass ? <PartyPopper size={40}/> : <Star size={40}/>}</div><h4 className="text-3xl font-black uppercase italic">{isPass ? 'KAMU LULUS! 🎓' : 'PAKET SELESAI! ✨'}</h4><div className="bg-white/10 p-5 rounded-2xl border border-white/20"><p className="text-[10px] font-bold uppercase tracking-widest leading-relaxed">✨ Kami sangat bangga dengan usahamu! Selamat atas pencapaian ini, Kak. Teruslah berkarya dan raih mimpi-mimpimu ya! ✨</p></div><button onClick={() => handleDownloadPDFReport(course)} disabled={activeDownloadId === course.id} className={`w-full py-6 bg-white ${isPass ? 'text-emerald-600' : 'text-orange-600'} rounded-[2.5rem] font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-4 active:scale-95 transition-all`}>{activeDownloadId === course.id ? <Loader2 className="animate-spin" /> : <Download size={24}/>} UNDUH RAPOT DIGITAL ✨</button><p className="text-[8px] font-black uppercase tracking-[0.3em] opacity-40">DITERBITKAN PADA: {formatDateToDMY(reportLog.date)}</p></div>) : (<>{isRejected && (<div className="bg-rose-50 p-6 rounded-3xl border border-rose-100 space-y-3 animate-in fade-in"><div className="flex items-center gap-3 text-rose-600"><AlertCircle size={20}/><p className="text-[10px] font-black uppercase">Permintaan Sebelumnya Ditolak</p></div><p className="text-[9px] font-bold text-slate-500 uppercase leading-relaxed italic">"Halo Kak! Permintaan rapotmu belum bisa diproses oleh pengajar yang sebelumnya dipilih. Jangan khawatir, silakan klik tombol klaim lagi dan pilih Guru Pembimbing lain ya! ✨"</p></div>)}<div className="grid grid-cols-2 gap-5">{[1, 2, 3, 4, 5, 6].map(sNum => { const doneLog = completedSessions.find(s => s.num === sNum); return (<button key={sNum} disabled={!!doneLog || (sNum !== maxSess + 1) || isProcessing} onClick={() => setConfirmingAbsen({course, sessionNum: sNum})} className={`p-6 md:p-8 rounded-[2rem] font-black transition-all border-4 flex flex-col items-center justify-center gap-2 ${!!doneLog ? 'bg-[#0F172A] border-[#0F172A] text-white shadow-xl' : (sNum === maxSess + 1 && !isProcessing) ? 'bg-white border-emerald-500 text-emerald-600 shadow-lg scale-105' : 'bg-slate-50 border-transparent text-slate-200'}`}><span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">{doneLog ? formatDateToDMY(doneLog.date) : `SESI ${sNum}`}</span>{!!doneLog ? <div className="flex flex-col items-center gap-1"><div className="w-8 h-8 bg-emerald-500 text-white rounded-full flex items-center justify-center"><Check size={18} strokeWidth={4}/></div><span className="text-[9px] font-black">SELESAI</span></div> : <span className="text-2xl">{sNum}</span>}</button>); })}</div><div className="space-y-4"><div className="h-4 bg-slate-100 rounded-full overflow-hidden p-1 shadow-inner"><div className="h-full bg-blue-600 rounded-full transition-all duration-1000 shadow-[0_0_15px_rgba(37,99,235,0.4)]" style={{ width: `${progressPercent}%` }}></div></div><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 text-center animate-pulse">{maxSess < 6 ? `Semangat ya! ${6 - maxSess} pertemuan lagi menuju rapotmu ✨` : 'Yeay! Sesi lengkap, silakan klaim rapotmu 🎓'}</p></div>{maxSess >= 6 && !isProcessing && !isReportPublished && (<button onClick={() => setRequestingReportFor(course)} className="w-full py-8 bg-slate-900 text-white rounded-[3rem] mt-6 font-black text-[12px] uppercase tracking-[0.4em] shadow-2xl hover:bg-black transition-all flex items-center justify-center gap-4 active:scale-95 animate-bounce"><GraduationCap size={28}/> {isRejected ? 'PILIH GURU LAIN ✨' : 'KLAIM RAPOT SEKARANG! 🎓'}</button>)}</>)}</div>
