...
const StudentPortal: React.FC<StudentPortalProps> = ({ 
  ...
  const [sessionActionModal, setSessionActionModal] = useState<any | null>(null); // { id, num, date } - pilihan Edit/Hapus, cuma buat sesi terakhir
  const [confirmDeleteSession, setConfirmDeleteSession] = useState<any | null>(null); // { id, num, date } - konfirmasi hapus sesi terakhir
...
setTimeout(() => {
  ...
  const executeDeletePayment = async () => {
    ...
    try {
      ...
      if (error) throw error;
      if (refreshAllData) await refreshAllData();
      // Kalau laporan yang barusan dihapus itu ternyata yang lagi diedit,
      // otomatis tutup mode edit — soalnya nggak ada lagi yang mau diedit.
      if (isEditing === confirmDeletePayment.id) {
        resetForm();
      }
  ...
  setLoading(true);
  try {
    // .select() dipasang biar kita tau persis baris yang KEBENERAN kehapus,
    // bukan cuma percaya "nggak ada error" — soalnya kalau RLS nolak diam-diam,
    // request ini tetap "sukses" tapi hasilnya 0 baris.
    ...
    if (!data || data.length === 0) {
      alert("Sesi nggak berhasil dihapus. Kemungkinan izin hapus data belum diaktifkan di sistem — coba minta Admin cek pengaturan database ya 🙏");
      setLoading(false);
      return;
    }
...
const handleDownloadPDFReport = async (course: any) => {
  ...
  const materiRowsFlexHtml = scores.map((score, i) => `
    <div style="flex:1; display:flex; align-items:center; justify-content:space-between; padding:0 35px; ${i < scores.length - 1 ? 'border-bottom:1px solid #f1f5f9;' : ''}">
      <span style="font-weight:900; color:#1e293b; font-size:20px; text-transform:uppercase; letter-spacing:-0.01em; line-height:1.1;">${topics[i] || 'MATERI PEMBELAJARAN'}</span>
  ...
  const materiCardsGridHtml = scores.map((score, i) => `
    <div style="background:white; display:flex; align-items:center; justify-content:space-between; gap:12px; padding:0 28px;">
      <span style="font-weight:900; color:#1e293b; font-size:16px; text-transform:uppercase; letter-spacing:-0.01em; line-height:1.15;">${topics[i] || 'MATERI PEMBELAJARAN'}</span>
  ...
  // ✅ ISI KOTAK MATERI - OTOMATIS 1 KOLOM FLEX-FILL ATAU GRID 2 KOLOM
  const materiBodyHtml = materiCount === 0
    ? `<div style="flex:1; display:flex; align-items:center; justify-content:center;"><span style="font-size:12px; font-weight:900; color:#cbd5e1; text-transform:uppercase; letter-spacing:0.2em;">Belum ada data materi</span></div>`
    : useGridLayout
      ? `<div style="flex:1; display:grid; grid-template-columns:1fr 1fr; grid-auto-rows:1fr; gap:2px; background:#f1f5f9;">${materiCardsGridHtml}</div>`
      : `<div style="flex:1; display:flex; flex-direction:column;">${materiRowsFlexHtml}</div>`;
...
<div class="page-wrapper">
  <div class="page-transkrip">
    ...
    <div style="background:white; border-radius:32px; border:3px solid #f1f5f9; overflow:hidden; margin-bottom:18px; flex:1; display:flex; flex-direction:column; min-height:0;">
      <div style="background:#0f172a; color:white; padding:10px 14px; text-align:center; flex-shrink:0;">
        <span style="font-size:10px; font-weight:900; text-transform:uppercase; letter-spacing:0.2em;">Materi & Nilai</span>
      </div>
      ${materiBodyHtml}
    </div>
  ...
  const handleDownloadSlipDirect = async (p: StudentPayment) => {
    ...
    try {
      ...
      pdf.line(marginL, y - 3, marginL + 4, y - 3);
      pdf.line(marginL, y - 1, marginL + 4, y - 1);
      pdf.line(marginL, y + 1, marginL + 4, y + 1);
...
>
  ...
  <div className="flex items-center gap-8 relative z-10"><div className="w-16 h-16 bg-orange-50 text-orange-600 rounded-3xl flex items-center justify-center shadow-inner shrink-0"><Receipt size={36} /></div><div><h3 className={`text-3xl font-black uppercase italic leading-none ${isEditing ? 'text-orange-600' : 'text-slate-800'}`}>{isEditing ? 'Edit Laporan' : 'Lapor Bayar'}</h3><p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-2">Pastikan bukti transfer jelas ya ✨</p></div>{isEditing && <button onClick={resetForm} className="ml-auto p-4 bg-rose-50 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"><X size={20}/></button>}</div>
                    ...
                    {payForm.receiptData ? (
                      ...
                      <div className="w-full">
                        ...
                        >
                          ...
                          <span className="text-xs font-bold text-slate-400 truncate text-left">
                                       ...
                                       {!!doneLog && !isRequesting && !isProcessing && !isNextClass && !isRejected && !isWaitingRelease && (
                                          <button 
                                            onClick={(e) => { 
                                              e.stopPropagation(); 
                                              if (sNum === maxSess) {
                                                setSessionActionModal(doneLog); // sesi terakhir: bisa pilih Edit atau Hapus
                                              } else {
                                                setEditDateValue(doneLog.date); setShowEditDateModal(doneLog); // sesi lama: langsung Edit aja
                                              }
                                            }} 
                                            ...
                                            title={sNum === maxSess ? "Kelola Sesi" : "Edit Tanggal Sesi"}
                                          >
      ...
      {confirmDeletePayment && (
        ...
        <div data-modal-container className="fixed inset-0 z-[120000] flex items-center justify-center p-6 bg-slate-900/90 backdrop-blur-xl opacity-0" style={{animation: 'modalFadeIn 0.3s ease-out forwards'}}>
           <div className="bg-white w-full max-w-[340px] rounded-[2rem] p-8 text-center space-y-6 shadow-2xl relative border-t-4 border-rose-500 opacity-0" style={{animation: 'modalZoomIn 0.3s ease-out 0.1s forwards'}}>
              ...
              <div className="w-16 h-16 bg-rose-50 text-rose-600 rounded-[1.5rem] flex items-center justify-center mx-auto shadow-sm animate-bounce"><AlertTriangle size={32} /></div>
              <div className="space-y-2"><h4 className="text-xl font-black text-slate-800 uppercase italic leading-none">Hapus Laporan?</h4><p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed px-4">Data laporan bayar <span className="text-slate-800 font-black underline">{confirmDeletePayment.className}</span> akan dihapus.</p></div>
